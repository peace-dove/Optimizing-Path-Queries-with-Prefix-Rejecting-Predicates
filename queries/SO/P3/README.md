...
Uses the GQL/SW (sliding-window) syntax to express temporal constraints declaratively. All queries match alternating `Rel` edges with even-length paths (1..4 hops). Queries are templates; start/end node IDs (`{start_id}`, `{end_id}`) are substituted from `../params/Q{5,6,7,8}-{single,pair}.txt`.
